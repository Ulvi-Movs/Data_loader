...
datasets_of_haberman = {
    "Conn-bench": 151,
    "statlog-heart": 145,
    "vertebral-column2": 212,
    "primary-tumor": 83,
    "horse-colic": 47,
    "cylinder-bands": 32,
...
medium_datasets = ["eye_movements", "gesture", "blastchar", "shrutime", "pol", "compass", "KDDCup", "electricity"]
...
def load_dataset(dataset_name) -> Tuple[data.Dataset, data.Dataset]:
    ...
    if dataset_name == "MNIST":
        ...
        mods = [
            transforms.Normalize((X.mean()), (X.std())),  # mean and std of MNIST
        ...
        train_dataset = data.TensorDataset(
            torch.tensor(np.array(X_train)).type(torch.FloatTensor), torch.tensor(np.array(y_train))
        )
        test_dataset = data.TensorDataset(
            torch.tensor(np.array(X_test)).type(torch.FloatTensor), torch.tensor(np.array(y_test)).type(torch.FloatTensor)
        )
        return train_dataset, test_dataset
    ...
    if dataset_name == "TUANDROMD":
        ...
        train_dataset = data.TensorDataset(
            torch.tensor(np.array(train_df)).type(torch.FloatTensor), torch.tensor(np.array(train_labels).astype(int))
        )
        test_dataset = data.TensorDataset(
            torch.tensor(np.array(test_df)).type(torch.FloatTensor), torch.tensor(np.array(test_labels).astype(int))
        )
        return train_dataset, test_dataset
        ...
        with requests.Session() as s:
            ...
            train_dataset = data.TensorDataset(
                torch.tensor(np.array(X_train)).type(torch.FloatTensor), torch.tensor(np.array(Y_train).astype(int))
            )
            test_dataset = data.TensorDataset(
                torch.tensor(np.array(X_test)).type(torch.FloatTensor), torch.tensor(np.array(Y_test).astype(int))
            )
        ...
        ionosphere = pd.read_csv(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/ionosphere/ionosphere.data", header=None
        )
        ...
        libras = pd.read_csv(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/libras/movement_libras.data", header=None
        )
        ...
        lymphography = pd.read_csv(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/lymphography/lymphography.data", header=None
        )
        ...
        dataset = pd.read_csv(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/glass/glass.data", header=None, na_values="?"
        ).dropna()
        X = dataset.values[:, :-1].astype(float)
        y = dataset.values[:, -1].astype(int)
        y = LabelEncoder().fit_transform(y).astype(int)
        ...
        dataset = pd.read_csv(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/00233/CNAE-9.data", header=None, na_values="?"
        ).dropna()
        X = dataset.values[:, 1:].astype(float)
        ...
        train_dataset = data.TensorDataset(
            torch.tensor(X_train).type(torch.FloatTensor), torch.tensor(y_train).type(torch.FloatTensor)
        )
        test_dataset = data.TensorDataset(
            torch.tensor(X_test).type(torch.FloatTensor), torch.tensor(y_test).type(torch.FloatTensor)
        )
        ...
        test_dataset = data.TensorDataset(
            torch.tensor(X_test).type(torch.FloatTensor), torch.tensor(y_test.values).type(torch.FloatTensor)
        )
        ...
        if dataset_name == "blastchar":
            ...
            arff_file = arff.loadarff("vistabnet/data/pol.arff")
            df = pd.DataFrame(arff_file[0])
            y = df["binaryClass"]
            ...
            arff_file = arff.loadarff("vistabnet/data/compass.arff")
            df = pd.DataFrame(arff_file[0])
            y = df["is_recid"]
            ...
            arff_file = arff.loadarff("vistabnet/data/KDDCup.arff")
            df = pd.DataFrame(arff_file[0])
            y = df["UPSELLING"]
            ...
            arff_file = arff.loadarff("vistabnet/data/electricity.arff")
            df = pd.DataFrame(arff_file[0])
            y = df["class"]
